...
st.title("📞 Adhérence Planning & Suivi Comportemental — CSA")
...
with tab1:
    ...
    c1.metric("Présents", int(nb_presents))
    c2.metric("Absents non justifiés", int(nb_absents))
    c3.metric("Cas à recadrer", int(nb_recadrer))
    c4.metric("Cumul retards plateau (min)", f"{cumul_retards:.0f}")
    c5.metric("Cumul dépassements pause (min)", f"{cumul_depassements:.0f}")
    ...
    st.dataframe(df_affiche[display_cols], use_container_width=True, hide_index=True)
...
with tab3:
    ...
    if login_fiche:
        ...
        st.markdown(f"**Statut du jour :** {row['Statut']}")
        ...
        with col_a:
            ...
            st.metric("Retard", f"{retard:.0f} min", delta=None if retard == 0 else f"+{retard:.0f} min")
        with col_b:
            st.markdown("#### ☕ Pauses")
            ...
            st.metric(
                "Dépassement",
                f"{depassement:.0f} min",
                delta=None if depassement == 0 else f"+{depassement:.0f} min",
            )
        with col_c:
            st.markdown("#### 🏁 Fin de poste")
            st.write(f"Prévu : {row['Fin_prevu']}")
            st.write(f"Réel : {row['Depart_reel']}")
            if row["Depart_anticipe_ignore"]:
                st.info("Non évalué (export temps réel)")
            else:
                depart_anticipe = row["Depart_anticipe_min"] or 0
                st.metric(
                    "Départ anticipé",
                    f"{depart_anticipe:.0f} min",
                    delta=None if depart_anticipe == 0 else f"-{depart_anticipe:.0f} min",
                )
